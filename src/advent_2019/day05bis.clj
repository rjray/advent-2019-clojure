...
;;; "Upgraded" version of day05.clj with a bug fix and (hopefully) some
;;; optimizations.
...
(defn- apply-out [state code]
  (let [{ops :op
    ...
    (make-state ops input (cons out output) (+ pc 2))))
  ...
  (loop [state state]
    (let [ops  (:op state)
      ...
      (cond
        ...
        (= op 4)  (recur (apply-out state code))
